...
;; Game -> Game
;; produce the next s
(check-random (tock-game G2)
              (make-game
               (bomb-loinvader
                (if (< (random INVADE-RATE) 50)
                    (cons (make-invader (random WIDTH) 0 10) (list I1.1))
                    (list I1.1))
                (list M1.1))
               (bomb-lomissile
                (if (< (random INVADE-RATE) 50)
...
(check-random (tock-loinvader LOI)
              (if (< (random INVADE-RATE) 50)
...
(check-random (just-spawn-loinvader LOI)
              (if (< (random INVADE-RATE) 50)
...
(check-expect (tock-lomissile (list M1)) (list M1.1))
...
(check-expect (out-screen? M5.1) true)
(check-expect (out-screen? M5) false)
...
(check-expect (bomb-invader? I1 (list M1 M2 M3)) true)
(check-expect (bomb-invader? I4 (list M1 M2 M3)) false)

(define (bomb-invader? i lom)
...
(check-expect (bomb-missile? M2 (list I4 I5 I6 I1)) true)
(check-expect (bomb-missile? M2 (list I4 I5 I6)) false)

(define (bomb-missile? m loi)
